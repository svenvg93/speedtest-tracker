...
it('does not include a threshold dataset when absolute thresholds are disabled', function () {
    Result::factory()->count(3)->create(['download' => 100000000]);
    ...
    $component = Livewire::test(RecentDownloadChartWidget::class, ['pageFilters' => $this->pageFilters])->instance();
...
it('includes a flat threshold dataset matching the configured absolute download threshold', function () {
    Result::factory()->count(3)->create(['download' => 100000000]);

    tap(app(ThresholdSettings::class), function (ThresholdSettings $settings) {
        $settings->absolute_enabled = true;
        $settings->absolute_download = 50.0;
        $settings->save();
    });
    ...
    expect($threshold)->not->toBeNull()
        ->and($threshold['data'])->toEqual(array_fill(0, 3, 50.0));
});
