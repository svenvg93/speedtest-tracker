...
it('does not include a threshold dataset when absolute thresholds are disabled', function () {
    Result::factory()->count(3)->create(['ping' => 15]);
    ...
    $component = Livewire::test(RecentPingChartWidget::class, ['pageFilters' => $this->pageFilters])->instance();
...
it('includes a flat threshold dataset matching the configured absolute ping threshold', function () {
    Result::factory()->count(3)->create(['ping' => 15]);

    tap(app(ThresholdSettings::class), function (ThresholdSettings $settings) {
        $settings->absolute_enabled = true;
        $settings->absolute_ping = 30.0;
        $settings->save();
    });
    ...
    expect($threshold)->not->toBeNull()
        ->and($threshold['data'])->toEqual(array_fill(0, 3, 30.0));
});
